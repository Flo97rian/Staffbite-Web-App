import store from "../../../../store";
      ...
      export function addNewShiftToImportedShiftPlan({ index, Plans, currentShiftPlan, daysIsActive }) {
  if (daysIsActive !== null) {
    var shiftname = "rolle" in daysIsActive;
    var beginn = "beginn" in daysIsActive;
    var ende = "ende" in daysIsActive;
    if (shiftname && beginn && ende) {
      var plans = getCopyOfShiftPlans({ Plans });
      var currentPlan = getCurrentPlan({ currentShiftPlan, Plans });
      var element = getCurrentPlanLength(-1, currentPlan);
      var input = createNewShiftRow({ plans, daysIsActive, currentShiftPlan });
    ...
    export function addShiftToNewShiftPlan({ index, NewShiftPlan, daysIsActive }) {
  if (daysIsActive !== null) {
    ...
    store.dispatch({ type: "SetNewShiftPlan", payload: shiftplan });
      ...
      const createMondayToSunday = ({plans, currentShiftPlan, daysIsActive}) => {
        let WeekDaysDetails = {};
        if (daysIsActive !== null) {
          const anzahl = "anzahl" in daysIsActive;
          if (anzahl) {
