    ...
    for (const [key] of Object.entries(plan[index])) {
      if (key !== "Wochentag") {
      let includesAnzahl = "anzahl" in daysIsActive ? daysIsActive.anzahl : plan[index][key]["anzahl"];
    ...
    plan[index].Wochentag["ShiftName"] = shiftname ? daysIsActive.rolle : plan[index].Wochentag["ShiftName"];
    plan[index].Wochentag["ShiftStart"] = beginn ? daysIsActive.beginn : plan[index].Wochentag["ShiftStart"];
    plan[index].Wochentag["ShiftEnd"] = ende ? daysIsActive.ende : plan[index].Wochentag["ShiftEnd"];
    return plan;
    };