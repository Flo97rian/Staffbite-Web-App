...
const setNewShiftPlanShifts = (daysIsActive) => {
    let schichtentag = daysIsActive["schichtentag"]
    return schichtentag
  }
  ...
  const addDatesToNewShiftPlan = (shiftplan, state) => {
    ...
    let datearray = {}
    dates.forEach(day => { day !== "Wochentag" ? datearray[day] = moment(state.date.start, "DD.MM.YYYY").add(dates.indexOf(day) - 1, "d").locale("de").format("l") : datearray[day] = "Datum"});
