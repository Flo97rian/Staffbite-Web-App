...
const FormSchichtBearbeiten = (props) => {
    const Plans = props.plaene
    const currentPlan = props.shift
    const row = Number(props.modalkey)
    ...
    const shiftDetails = props.import ? props.plaene[props.shift].plan[row].Wochentag : props.Schichtplan[row].Wochentag
    const anzahl = props.import ? props.plaene[props.shift].plan[row].Montag : props.Schichtplan[row].Montag
        return(
            <>
                <Row>
                    <Col xs={1} ></Col>
...
export default FormSchichtBearbeiten